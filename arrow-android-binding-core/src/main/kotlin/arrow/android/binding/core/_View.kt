...
fun <V : View> V.longClicks(handle: (V) -> Boolean = AlwaysHandle): Stream<Unit> =
    Stream.cancellable {
        setOnLongClickListener { emit(Unit); handle(this@longClicks) }
        CancelToken { setOnLongClickListener(null) }
    }

fun <V : View> V.keys(handle: (V, Int, KeyEvent) -> Boolean = AlwaysHandle): Stream<OnKey> =
    Stream.cancellable {
...
fun <V : View> V.drags(handle: (DragEvent) -> Boolean = AlwaysHandle): Stream<DragEvent> =
    Stream.cancellable {
...
fun <V : View> V.hovers(handle: (MotionEvent) -> Boolean): Stream<MotionEvent> =
    Stream.cancellable {
...
fun <V : View> V.visibilities(): Stream<Visibility> = Stream.cancellable {
    setOnSystemUiVisibilityChangeListener { emit(Visibility(it)) }
